/**
 * Pure podcast-side derivation, shared by the browser and the server list
 * functions (Pass L2b). No React, no Supabase client.
 */
import type { MovieEntry } from "./entries";
import type { Catalog, Episode, Podcast, PodcastMetric } from "./types";
...
/** Summary row for the Shows list — counts only, no nested movie/episode arrays. */
...
export function buildPodcastEntries(catalog: Catalog, entries: MovieEntry[]): PodcastEntry[] {
...
export function toPodcastSummary(entry: PodcastEntry): PodcastSummary {
  return {
    ...
    streamableCount: entry.streamableUnwatched.length,
