...
#### Pass U76 — Repeatable admin verification identity — S (~1-2 credits) — blocks verification of U8, U32, U33, U38, U39, L2b, U63

Root cause of the recurring "IMPLEMENTED, NOT VERIFIED" labels: admin access is real at every layer (`user_roles` + `has_role`, RLS policies, server-side `Forbidden: admin required` after `requireSupabaseAuth`, with `useIsAdmin` only gating render), and the verification browser reports `signed_out`. Decision: keep the production model; do **not** weaken permissions (Option 1 rejected — policy/guard/client edits plus a restore pass, and verification would then prove behaviour that is not shipped). Chosen approach is the credential-free variant of Option 2: a dedicated non-personal admin identity (`verify-admin@…` auth user + `admin` row in `user_roles`), verified per turn by minting a short-lived real session for that user id and restoring it in the browser, so the check travels the real JWT → `requireSupabaseAuth` → RLS → `has_role` path. No stored credential, nothing in source or Project Knowledge, no bypass, no new test subsystem. Scope: create the identity and role row, record its user id plus the standing procedure in `workflow-documentation.md`, and add the rule that an admin-only item may only be labelled "IMPLEMENTED, NOT VERIFIED" for session reasons after that procedure has actually failed, with the failure quoted. Acceptance: with a minted session `/admin/ingest` loads, `useIsAdmin` is true and one admin-only write (Confirm) succeeds; signed out the control is absent and the server function still refuses. Build before the next admin-only pass needs verification.
...
#### Pass U78 — Admin RPC execute identity in production — S (~1-2 credits) — from Project monitoring triage 2026-09-10; re-confirmed by Project monitoring finding `error_log_finding_1eda9e944e4620fec6ef58c2abf4f6a2` (2026-09-11) — no separate item; verification depends on U76

Production error logs reported `permission denied for function admin_unlinked_episodes`, `admin_unlinked_episode_counts`, `admin_podcast_coverage`. Verified: all four `admin_*` helpers are `SECURITY INVOKER` with ACL `postgres=X, service_role=X` only (migrations 20260909044509 / 20260909045039), and all call sites use `supabaseAdmin` (`ingestion.functions.ts:1111, 1664, 1671, 1720`). The preview environment works — signed-in unmatched search returned results in ~1.3s on 2026-09-09 — and no `permission denied` rows remain in the retained Postgres log window, so this is environment-specific: the deployed `SUPABASE_SERVICE_ROLE_KEY` is very likely not a secret/service-role key, so the RPC executes as `anon`/`authenticated`. Options: (a) confirm/repair the published environment's service-role secret (preferred; no schema change); (b) make each helper `SECURITY DEFINER` with an internal `has_role(auth.uid(),'admin')` check and `GRANT EXECUTE ... TO authenticated`, which removes the dependency on key identity entirely. Acceptance: on the published site the ingestion stats, unmatched list and coverage cards render with real numbers and no `permission denied` appears in the logs for a full admin session.
...
#### Pass U80 — Restore source-side active-show filter on the catalogue episode read — S (~1-2 credits) — from Project monitoring finding `eafb27f3-a96f-5cb5-9177-01b0ef28fe82` (filed 2026-09-11)
