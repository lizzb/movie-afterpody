...
- **Pass L2c — Finish the server-side migration — M (~3-5 credits) — backlog, depends on L2b.** Move movie detail, Lists and Settings onto bounded server reads so `useCatalog` can be retired; slim the show-detail covered-movie payload; persist the taste profile to the account (upsert on change + merge on sign-in) so ranking follows the user across devices.
