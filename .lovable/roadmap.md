...
Current functionality (2026.08.25) -- Not Interested: it only stores that movie’s slug in your local app preferences; Tonight always excludes it, Movies can show/hide it with “Hide not interested,” and undo snackbar can reverse it.
...
- **Pass U9 — Learning evidence over time — M (~3-5 credits) — Priority 3.** Persist each Score the matcher run and show current vs previous with deltas and a short history. One migration. (NOTE: U9 becomes much more useful after you are generating substantial human decisions through the new review workflow.)
...
(Pass U36 additional information: `.lovable/plan/match-review-ux-filter-feedback-backlog-2026-09-02.md`)
...
#### Pass U56 — Token distinctiveness and missing-token penalty — M (~3-5 credits) — depends on U55 — SHIPPED 2026-09-14, VERIFIED
...
Built: catalogue-wide word distinctiveness (`computeTitleWordStats`, new `titleWordStats` option) so a small candidate pool scores exactly as production does; weighted coverage; post-colon chatter weighted at 0.4 with a `chatterOnly` cap; missing-distinctive penalty scaled by IDF, hardened to a cap when the omitted word is more identifying than anything the episode says ("Falling for Figaro" for "Falling for You", "The Parent Trap" for "Trap"); sub-phrase rule so a candidate that is only part of a longer phrase is not the subject ("Friday" in "Friday Night Lights", "Drive" in "License to Drive"), with ordinary chatter around a real title exempt ("Magnolia (Dads Can Be Very a Lot)"); an exact whole-title hit stands other containment candidates aside. New signals `weightedCoverage` / `missingDistinctive` / `chatterOnly` surfaced in the matcher scorecard. `scripts/matcher-corpus.ts` and the evaluator now both score over catalogue-wide statistics.

Acceptance checklist — **Verified:** all 12 U56-owned corpus cases pass (`bun scripts/matcher-corpus.ts`), 26 of 29 judged cases pass overall with the 3 remaining failures attributed to their owning future passes (U58 ×1, U59 ×1, U60 ×1) and 3 skipped for absent catalogue titles; scoring over 10,778 labelled pairs at threshold 25 — precision 60.02% → 87.82%, recall 98.36% → 96.67% (drop 1.69 points, inside the ≤2-point gate); typecheck clean. **Not applicable:** no schema, UI or write-path changes; no per-show exceptions; no replay/recheck run.

#### Pass U57 — Description title+year evidence and the year gate — M (~3-5 credits) — depends on U56 — IMPLEMENTED, NOT VERIFIED 2026-09-14 (accuracy gate not met)
...
Built: description title mentions are detected before the year is scored; years now count only when they sit in the same sentence as the mention, and a multi-word title named beside its own release year lifts a description-only match to a 72 floor (`DESC_TITLE_YEAR_FLOOR`), strictly stronger than the old flat bonus. Year gate: a year adds nothing unless the candidate already has lexical evidence (exact hit, containment with shared words, full coverage, or a shared word at or above `IDENTIFYING_IDF` 0.65) or a description mention. Also fixed a latent string-containment bug where an episode title shorter than four canonical characters (`π`) was "contained" in every movie title. New signals `descTitleYear` / `yearGated` surfaced in the matcher scorecard. Corpus grew three U57 cases.
...
**As built (2026-09-15):** `src/lib/podcast-profile.ts` (pure, corpus-testable) + `src/lib/podcast-profile.server.ts` (DB read). Profiles built from settled links only (`review_state='confirmed'` or `match_method='manual'`), one film per show however many episodes cover it, null below `PROFILE_MIN_SAMPLE = 8` with `PROFILE_MIN_COVERAGE = 0.6` per dimension. Penalties: genre −4 when the summed profile familiarity of the candidate's genres is below `UNUSUAL_GENRE_SHARE = 0.25` (summed rather than best-of, so one horror-comedy in a horror show does not excuse a rom-com), era −3 when the candidate's decade and both neighbours are unrepresented, certification −2 when the show has never carried it, total capped at `PROFILE_PENALTY_CAP = 8`, applied as `max(0, confidence − penalty)`. Exempt when the rule is `exact`, `descTitleYear` or `multiTitle`. New signals `unusualGenre` / `unusualEra` / `unusualCertification` / `profilePenalty`, surfaced in the scorecard. Wired into the unmatched-list and recheck matcher paths and the evaluator; the feed-sync auto-link path is intentionally excluded (no `podcast_id` in its episode shape, and profiling auto-links would reinforce the matcher's own mistakes).

Acceptance — **Verified**: 4 new U72 corpus cases pass (rom-com on a horror show loses 4 points yet still wins and stays linkable; a 2025 film on a throwback show loses 3 points yet stays linkable; the same 2025 film named exactly loses nothing; a 3-film show has no prior at all), 46/46 judged corpus cases pass with no regressions (8 skipped for films absent from the catalogue); over the 10,779 human-labelled pairs precision 89.45% → 89.53% and recall 96.10% → 96.17% — both improved, gate met; typecheck clean; no write path, schema or stored-confidence change.
...
#### Pass U75 — Show-format regression corpus and format-prefix coverage — M (~3-5 credits) — depends on U55; gates U55–U62 acceptance — SHIPPED 2026-09-14, VERIFIED
...
Implementation: corpus data in `src/lib/matcher-corpus.ts` (32 cases, each labelled with the owning pass, `expect` / `forbid` / `expectNoLink`); runner `scripts/matcher-corpus.ts` — `bun scripts/matcher-corpus.ts [PASS …]` — loads only the named candidate films from the catalogue, scores them with the live matcher, prints PASS / FAIL / SKIP per case with the winning candidate and its full signal set, and tallies by owning pass. Read-only: no writes to links, rejections or review states. Cases whose expected film is absent from the catalogue report SKIP rather than counting as matcher failures. Parse vocabulary added to `src/lib/providers/episode-parse.server.ts`: format prefixes (`BRUNCH:`, `Redux:`, `Micro Queers:`, `Matinee Monday:`, `Last Looks:`, `DTH Classic:`, `Interview:`, `Feed Drop -`, `FH Mini 117 –`, `Ep. #441 -`, mailbag / Q&A), format suffixes (`LIVE!`, `(LIVE from …)`, `(Patreon Clip)`, `(Classic)`, `(Re-Release)`, `[Jason Edition]`, channel-date tags such as `(Hallmark+ - 2026)`), leftover-bracket cleanup, and `Interview: <guest> on <title>` splitting (only after an interview prefix, so ordinary titles containing " on " are untouched).

Acceptance checklist — **Verified:** one command reports per-case pass/fail with winner and signals; all 5 U55-owned and the U75-owned format-prefix case pass; 13 of 29 judged cases pass overall with the remaining failures attributed to their owning future passes (U56 ×11, U58 ×2, U59 ×1, U60 ×2) and 3 skipped for absent catalogue titles; matcher scoring over 10,778 labelled pairs improved again after the new vocabulary — precision at threshold 25 59.77% → 60.02%, recall 98.27% → 98.36%; typecheck clean. **Not applicable:** no schema, UI or write-path changes; no per-show exceptions; no replay/recheck run.
...
#### Pass U89 — Catalogue read fan-out and payload weight — M (~3-5 credits), confidence Low (measure first) — PARTIALLY SHIPPED 2026-09-20, VERIFIED (episode fan-out); remainder DEFERRED — plan: `.lovable/plan/plan-backlog-only-pass-u89-catalogue-read-fan-out-and-payloa-2026-09-18.md`

Residual risk left over from the 2026-09-17 index fix (which removed the whole-table episode sort and the visitor-facing timeouts). Two app-level causes remain: (1) fan-out — one page load can trigger several overlapping catalogue reads (6 large requests at once observed) and repeat loads re-request the same rows; (2) payload weight — the episode read selects full descriptions even on surfaces that render only title, date and links. Scope: measure call counts and bytes per load for `/`, `/movies`, `/podcasts`, a show page and a movie page (warm and cold) and record them before changing anything; collapse duplicate concurrent reads into one in-flight request per key; split the episode read into a lightweight projection for browse/list/detail and keep the full projection for matcher and admin review; confirm existing TTL behaviour still holds, no second cache layer. Out of scope: cold-read latency and retiring `src/lib/data.ts` (L2c-1), availability freshness (U67/U68), schema changes, new indexes, matcher scoring changes, any visible change. Depends on / must not regress U79 and U80; overlaps L2c-1 on the same reader (re-measure if L2c-1 lands first). Unknowns: whether the 66k call count is visitor traffic, ingestion/rescan loops, or repeated preview loads (changes the fix entirely); how many surfaces consume `description`; whether collapsing alone suffices. Acceptance (pre-build): before/after call counts and bytes recorded; duplicate concurrent reads for one key reduced to a single request; browse/list/detail no longer transfer descriptions; matcher precision/recall unchanged on the U75 corpus; no visible change and no console errors at 390px and 1280px. Also absorbs U40H (2026-09-20): the Podcast Show details Movies / Episodes toggle should load only the data its active view needs — same reader, so U89 owns it.

Measured 2026-09-20 (CREDITLIMIT=2, so measurement was scoped to the episode reader rather than full per-page byte accounting). 40 active shows; largest show 898 episodes; no show above the 1000-row page size. `fetchEpisodesByPodcast` called `fetchAllRows`, which fires a wave of 6 parallel 1000-row windows per show, so every catalogue read issued ~240 episode queries of which ~200 returned zero rows — this, not visitor traffic, is the dominant source of the recorded call count. Two plan items were already satisfied before this pass: the catalogue episode projection does not select `description`, and `loadCatalog()` already collapses concurrent reads into one in-flight promise behind the 5-minute snapshot.

Shipped: `src/lib/catalog.server.ts` — new `fetchEpisodesForPodcast` reads one show's episodes sequentially page by page and stops as soon as a page comes back short, keeping the existing `readWindow` retry/slice behaviour and the 8-show concurrency. Episode queries per cold catalogue read drop from ~240 to 40 (~6x), with identical rows and ordering. Verified 2026-09-20 (local, 1280px, cold snapshot after a dev-server restart): `/` renders Tonight rows, `/movies` reports 3784 movies with rows, `/podcasts` renders the show list, `/podcasts/messed-up-movies?tab=episodes` renders 49 link rows; no console errors; `bunx tsgo --noEmit` clean. No schema, cache-layer or visible change.

Deferred remainder (still U89): per-page byte accounting for `/`, `/movies`, `/podcasts`, movie and show pages (warm and cold); the same wasted-wave pattern on the other `fetchAllRows` callers (movies, availability, links), where the fixed wave of 6 over-fetches smaller tables; and U40H's conditional Movies/Episodes loading.
