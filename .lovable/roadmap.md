...
- **Pass L2a — Stop transferring what is never shown — SHIPPED 2026-09-05.** Episode `description`, movie `synopsis` and all 14,318 `episode_sources` rows left the catalogue read and now load per visible row / per movie via `src/lib/details.ts`; parked shows are excluded in the query (`.neq('curation_status','parked')` + `.in('podcast_id', activeIds)`); holiday exclusion moved to a server-side word-boundary match (`holidayMovieIds`); the two keyless `invalidateQueries()` calls in `episode-reviews.ts` are now a targeted key list. Acceptance: **Verified** — catalogue payload 8.0 MB decoded (was ~14 MB with descriptions/sources), `episode_sources` requests gone, first content 5-6 s dev-cold on `/movies`, `/podcasts`, show detail; descriptions and synopses still render. **Needs follow-up:** remaining 8 MB is episodes 3.2 / links 1.7 / availability 1.5 / movies 1.0 / genres 0.7 MB — only L2b removes those.
